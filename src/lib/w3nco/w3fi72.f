...
C
      ...
      CHARACTER * 1   PDS(*)
      CHARACTER * 1   GDS(200)
      CHARACTER(1),POINTER:: BMS(:), BMSBIG(:)
      CHARACTER(1),POINTER:: PFLD(:)
      CHARACTER(1),POINTER:: IPFLD(:)
      CHARACTER * 1   SEVEN
      CHARACTER * 1   ZERO
      NULLIFY(BMS,PFLD,IPFLD)
C
C
...
C           PRINT *,' W3FI72 ERROR, IBLEN .NE. NPTS = ',IBLEN,NPTS
          ...
          ALLOCATE(BMSBIG(1000 + 9*NUMBMS + 1000))
          BMS => BMSBIG((1000+4*NUMBMS):(1000+5*NUMBMS-1))
          ZERO = CHAR(00)
          BMS = ZERO
...
C
 900  CONTINUE
      IF(ASSOCIATED(BMSBIG)) DEALLOCATE(BMSBIG)
      IF(ASSOCIATED(PFLD)) DEALLOCATE(PFLD)
      RETURN
      END
